...
<template>
  <div class="views">
    ...
    <el-row :gutter="4">
      <el-col :span="6">
        <el-select v-model="options[this.selectedIndex].USERID"  placeholder="请选择医生" size="small">
          <el-option @click.native="getValue(item.USERID,item.disabled)"
      ...
      <el-col :span="3">
        <el-button
          ...
          @click="gotoMedicalOrderApply()">返回</el-button>
      </el-col>
      <el-col :span="15">
        <el-input
          clearable
  ...
  <div class="block">
    ...
    <el-table-column width=200  label="支付管理">
      <template slot-scope="scope">
          <el-button
          size="mini"
          type="success"
    ...
    <el-table-column width=200  label="服务管理">
      <template slot-scope="scope">
          <el-button
          size="mini"
          type="success"
...
export default {
  ...
  data() {
    return {
      showButton:false,
      selectedIndex:0,
      COMMUNITYID:"cm2022082000000003",
      options:[],
      ban:false,
      access_token: null,
      tableData: [],
  ...
  methods: {
    ...
    tableRowClassName({row}) {
      ...
      cancelFollow(row){
        this.$http
          .delete(
            '/API/follow?FOLLOWERID='+row.ELDERID
            +'&FOLLOWINGID='+this.doctorid,
            {
              headers: {
                TokenValue: this.access_token,
              },
    ...
    FollowedOrNot(row){
      for(var i = 0; i < this.tableData2.length; i++){
        console.log("77777777777",row);
        if(row.ELDERID == this.tableData2[i].USERID){
          return true;
        }
      }
      return false;
    },
    getValue(userID,disabled){
         console.log("select a userID",userID,disabled);
         console.log("select a userID999999999999999999999999",userID,disabled);
         for(var i=0;i<this.options.length;i++){
          if(userID==this.options[i].USERID && disabled!=true){
      ...
      getCommunityID(){
        ...
        this.$http
          ...
          .then((result) => {
            ...
            this.COMMUNITYID = result.data[0].COMMUNITYID; 
            this.getDoctorList();
          })
          .catch((err) => {
            if(err.message === 'Request failed with status code 500' || err.message === 'Request failed with status code 403'){
      ...
      getDoctorList(){
        ...
        this.$http
          ...
          .then((result) => {
            ...
            for(var i=0;i<this.options.length;i++){
              console.log(this.options[i].BANTIME);
              if(this.options[i].BANTIME==null){
            ...
            for(var i=0;i<this.options.length;i++){
              if(this.showButton){
                if(this.doctorid == this.options[i].USERID){
                  this.selectedIndex = i;
                  break;
                }
              }else{
                if(this.options[i].BANTIME==null){
                  this.selectedIndex = i;
                  this.doctorid = this.options[i].USERID;
                  break;
                }
              }
            }
      ...
      getFollowList(){
        this.$http.request({
          url: "/API/Follow/followingID?followingID=" + this.doctorid,
          method: "get",     
          headers: {
                TokenValue: this.access_token,
          },
        ...
        .then(async (res) => {
          ...
          console.log("当前医生的关注列表",this.tableData2);
        })
        .catch((err) => {
          if (
              err.message === "Request failed with status code 500" ||
  ...
  mounted() {
      ...
      try{
        ...
        if(this.doctorid.length < 1){
          this.doctorid = '102022103000000001';
        }
          try{
