...
<template>
  <div class="placeCareOrder_box">
    <div class="order-header">呼叫界面</div>
    ...
    <div class="order-form-box">
      ...
      >
        ...
        <el-form-item class="f_total_price_box">
          ...
          <span class="price_num">{{ this.totalPrice }}元</span>
        </el-form-item>
      </el-form>
    ...
    <div class="button-box">
      <el-button
        class="step-button"
        v-on:click="back"
        v-show="order_step != 0 && order_step != 1 && order_step != 3"
        >上一步</el-button
      >

      <el-button class="step-button" v-on:click="exit" v-show="order_step == 0 || order_step == 1" 
        >返回</el-button
      >
      <el-button
        class="step-button"
...
export default {
  ...
  methods: {
    ...
    elderSubmit() {
          ...
          let submit = {
            ...
            order_time: this.form.place_time + " 0:00:00",
            address: "待操作员填写",
            status: "3",
          ...
          if (submit.serviceitems.length>0) {
            axios
              .request({
    ...
    formSubmit(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios
            .request({
              url: "/API/ServiceRecord?id="+this.serviceRecordID,
              method: "put",
              data: submit,
              headers: this.headers,
            })
  ...
  mounted: function () {
    ...
    this.backPageName = this.$route.params.backPageName;
    if(this.backPageName=="CareService"){
      this.order_step = 0;
      //获取上个页面传来的数据，保存
    ...
    }else if(this.backPageName=="CallService"){
      this.order_step = 1;
      //获取上个页面传来的数据，保存
      this.form.shopping_cart = this.$route.params.careServiceDetail.service_items;
      this.form.category_id = this.$route.params.careServiceDetail.categoryID;
      this.form.place_time= this.$route.params.careServiceDetail.place_time;
      this.totalPrice = this.$route.params.careServiceDetail.total_price;
      this.form.category_name = this.$route.params.careServiceDetail.categoryName;
      this.serviceRecordID = this.$route.params.careServiceDetail.id;
