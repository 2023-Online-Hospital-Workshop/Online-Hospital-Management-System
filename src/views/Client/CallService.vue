  ...
  <div class="views">
    ...
    >
      ...
      <template slot-scope="scope">
          <el-button
          type="primary"
          @click="selectService(scope.row)">下单</el-button>
      </template>
      </el-table-column>
    ...
    selectService(row){
      ...
      if(row.categoryID!="6"){
        ...
        this.$router.push({
          name: "PlaceCareOrder",
          params: {
            careServiceDetail: row,
            backPageName:"CallService"
          },
        });
        console.log("----------");
      }else{
         ...
         let submit = {
            typeid: "6",
            start_time: row.place_time + " 0:00:00",
            order_time: row.place_time + " 0:00:00",
            address: "已无效",
            status: "4",
            price: 0,
            evaluationid: "",
          ...
          axios
            .request({
              url: "/API/ServiceRecord?id="+row.id,
              method: "put",
              data: submit,
        ...
        this.$router.push({
          name: "DoctorService",
          params: {backPageName:"CallService"},
        });
      }
    },
    //服务类型表头的筛选的函数
