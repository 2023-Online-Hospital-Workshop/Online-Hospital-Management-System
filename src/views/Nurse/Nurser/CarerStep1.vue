...
import { setToken, getNurseCard, getDoctorCard, getQualificationCard } from '../../../api/data.js'
...
export default {
  ...
  mounted: function () {
    ...
    if (this.getRole(uid) == "护工") {
      Axios.all([getNurseCard(uid,access_token), getQualificationCard(uid, access_token)])
      ...
      Axios.all([getDoctorCard(uid,access_token), getQualificationCard(uid,access_token)])
        .then(Axios.spread((resNurse, resQualification) => {
