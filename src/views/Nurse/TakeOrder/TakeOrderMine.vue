  ...
  <div class="views">
    <el-row :gutter="8">
      <el-col :span="7">
        ...
        <el-tabs v-model="activeName" type="card" class="tabs" @tab-click="HandleTabsClick">
          ...
          <el-tab-pane label="已分配" name="requested"></el-tab-pane>
      ...
      <el-col :span="10">
        <el-select v-model="options[this.selectedIndex].id"  placeholder="请选择护工">
          <el-option @click.native="getValue(item.id)"
    ...
    >
      ...
      <span slot="footer" class="dialogFooter">
        <el-button 
        type="danger" 
        v-if=" details.state === '已分配' || details.state === '分配失败'" 
        @click="CLickCancelRequestButton()" 
        :disabled="details.state === '分配失败'"
          >取消申请</el-button
        >
...
export default {
  ...
  methods: {
    getValue(userID){
        console.log("select a userID999999999999999999999999",userID);
        for(var i=0;i<this.options.length;i++){
          if(userID==this.options[i].id){
    ...
    getAllUser () {//返回值：所有用户的列表
      ...
      Axios.all([this.getAllNurse()])
        .then(Axios.spread((resNurse) => {
          console.log("后端请求成功!");
  ...
  mounted: function () {
    ...
    this.access_token = this.$cookies.get("token");
    this.options = this.getAllUser();
    console.log("this.options",this.options);
    if(this.options.length>0){
      this.my_uid = this.options[0].id;
    }
    this.load_all_data();
