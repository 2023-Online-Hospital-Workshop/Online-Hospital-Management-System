        ...
        <div class="diagnostic">
            ...
            <table id="info">
                ...
                <tr>
                    ...
                    <td> <va-button @click="showModal = !showModal">
                            查看就诊历史
                        </va-button></td>
                        ...
                        <div class="diagnose">
                            ...
                            <table id="info">
                                ...
                                <tr>
                                    ...
                                    <td>科室：{{ his_rep.records[p].doctor.secondaryDepartment }}</td>
                                    <td>医生：{{ his_rep.records[p].doctor.name }}</td>
                                </tr>
                                <div>
                                    主诉：{{ his_rep.records[p].record.selfreported }}
                                    <br><br>现病史： {{ his_rep.records[p].record.presenthis }}
                                    <br><br>既往史： {{ his_rep.records[p].record.anamnesis }}
                                    <br><br>体征：{{ his_rep.records[p].record.sign }}
                                    <br><br>门诊诊断： {{ his_rep.records[p].record.clinicdia }}
                                    <br><br>诊疗建议: {{ his_rep.records[p].record.advice }}
                                    <br><br>处方：
                                </div>
                                <div class="example-pagination-block">
                                    <el-pagination @current-change="changeReport" layout="prev, pager, next"
                                        :total="total_p" />
                                </div>
                            </table>
                            <label>
            ...
            <va-form class="w-[300px]" tag="form" @submit.prevent="enter">
                ...
                主诉： <input type="text" v-model="problem" name="firstname" placeholder="请输入">
                <br><br>现病史： <input type="text" v-model="illness" name="firstname" placeholder="请输入">
                <br><br>既往史： <input type="text" v-model="past_illness" name="firstname" placeholder="请输入">
                <br><br>体征：<input type="text" v-model="symptom" name="firstname" placeholder="请输入">
                <br><br>门诊诊断： <input type="text" v-model="diagnose" name="firstname" placeholder="请输入">
                <br><br>诊疗建议: <input type="text" v-model="prescription" name="firstname" placeholder="请输入">
                <br><br>处方:
                ...
                <el-table :data="medicine" stripe style="width: 100%" max-height="500">
                    ...
                    <el-table-column label="单次剂量" width="120">
                        <template #default="scope">
                            <input class="input2" type="text" v-model="all_med[scope.$index].single" name="firstname"
                                placeholder="">
                        </template>
                    </el-table-column>
                    <el-table-column label="用法" width="70">
                        <template #default="scope">
                            <input class="input2" type="text" v-model="all_med[scope.$index].ad" name="firstname"
                                placeholder="">
                        </template>
                    </el-table-column>
                    <el-table-column label="频率" width="100">
                        <template #default="scope">
                            <input class="input2" type="text" v-model="all_med[scope.$index].fre" name="firstname"
                                placeholder="">
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column label="注意事项" width="140">
                        <template #default="scope">
                            <input class="input2" type="text" v-model="all_med[scope.$index].tips" name="firstname"
                                placeholder="">
                        </template>
                    </el-table-column>
...
export default {
    ...
    methods: {
        //初始化今日挂号的病人
        async initial() {
            let api = "http://124.223.143.21:4999/Registration/commit?doctorId=" + this.doctorId;
            try {
        ...
        async nextPatient() {
            ...
            if (this.patients[i].gender) {  //gender是true则为男
                this.gender = "男";
            } else {
                this.gender = "女";
            }
        ...
        enter() {
            ...
            let a = "";
            for (let i = 0; i < this.all_num; i++) {
                a = a + this.all_med[i].name + "*" + this.all_med[i].count + "+" + this.all_med[i].fre + "#" + this.all_med[i].tips + ";";
            }
            let data = {
        ...
        //得到就诊历史
        async getHistory() {
            const url = "http://124.223.143.21/api/DiagnosedHistory/GetDetailPre?patientId=" + this.patients[this.num - 1].patientId;
